...
# [FIX] Menggunakan python3-full agar venv tidak error di Ubuntu 24.04
...
if [ ! -f "venv/bin/activate" ]; then
    echo -e "\033[0;31m[ERROR] Gagal membuat Venv. Cek koneksi internet atau space disk.\033[0m"
    exit 1
fi
...
# [FIX] Memastikan paket 'openai' dan 'watchdog' terinstall 
if [ -f "requirements.txt" ]; then
    echo "   - Menginstall dari requirements.txt..."
    pip install -r requirements.txt
    # Double check untuk library krusial
    pip install openai watchdog streamlit-option-menu
else
...
# [FIX] Tambahkan mode headless dan abaikan warning untuk kebersihan terminal
cat <<EOF > run.sh
#!/bin/bash
source $(pwd)/venv/bin/activate
export PYTHONWARNINGS="ignore"
cd $(pwd)
# Headless true mencegah error 'gio' dan mencegah browser terbuka otomatis di WSL
...
# Diagram Proses Startup:
# [Windows Boot] -> [Startup Folder] -> [.vbs file] -> [WSL] -> [run.sh] -> [Streamlit Server]



if [ -d "$STARTUP_PATH" ]; then
    VBS_FILE="$STARTUP_PATH/StartGeminiMetadata.vbs"
    # Menulis file VBScript: Parameter 0 artinya jendela terminal disembunyikan (SILENT)
    cat <<EOF > "$VBS_FILE"
...
echo -e "Aplikasi akan otomatis berjalan di background saat login Windows."
